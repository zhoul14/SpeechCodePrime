...
class GMMEstimator {
	...
	int sigmaL() {
		...
		{
			return DefaultFdim * DefaultFdim + (fDim - DefaultFdim) * (fDim - DefaultFdim);
		}
	...
	bool isFullRank() {
		return cbType != CB_TYPE_DIAG && cbType != CB_TYPE_FULL_MIX;
	}
